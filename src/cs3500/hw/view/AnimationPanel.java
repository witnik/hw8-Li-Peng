      ...
      for (IAnimation a : moves) {
        ...
        if (s.getName().equals(temp.getName())
          ...
          if (a instanceof Move) {
            ...
            width = s.getWidth() + (((Scale) a).getNwidth() - s.getWidth())
                    / (float) (a.getEnd() - currentTime + 1);
            height = s.getHeight() + (((Scale) a).getNheight() - s.getHeight())
                    / (float) (a.getEnd() - currentTime + 1);
          } else {
            red = s.getRed() + (((ChangeColor) a).getRed() - s.getRed())
                    / (float) (a.getEnd() - currentTime + 1);
            green = s.getGreen() + (((ChangeColor) a).getGreen() - s.getGreen())
                    / (float) (a.getEnd() - currentTime + 1);
            blue = s.getBlue() + (((ChangeColor) a).getBlue() - s.getBlue())
                    / (float) (a.getEnd() - currentTime + 1);
          }
        }
      }
